...
public final class RatTerm {
   ...
   */
  public RatNum getCoeff() {//get the coefficient of this RatTerm  
    return coeff;
  }
   ...
   */
  public int getExpt() {    
    if(coeff.equals(RatNum.ZERO)){// if coefficient is zero, then expt is 0
    	return 0;
    }else{
    	return expt;
    }
  ...
  public boolean isNaN() {    
    return coeff.isNaN();// if coefficient is NaN, then the RatTerm is NaN 
  }
   ...
   */
  public boolean isZero() {// if coefficient is 0, then the RatTerm is 0 
  ...
  public double eval(double d) {
    if(isNaN()){// if it is NaN, return Double.NaN
    	return Double.NaN;
    }else{        //else return the value when evaluated at d
  ...
  public RatTerm negate() {
	  if(isNaN()){
		  ...
		  return new RatTerm(coeff.negate(),expt);//negate the coefficient
	  }
  }
  ...
  public RatTerm add(RatTerm arg) {
	  if((expt!=arg.expt)&&!(isNaN()||isZero())&&!(arg.isNaN()||arg.isZero())){// throw exception if two 
		  throw new IllegalArgumentException(); // degrees are different, and neither RatTerm is zero or NaN
	  }else if(isNaN()||arg.isNaN()){
		  return new RatTerm(RatNum.NaN, 0);//If either argument is NaN,returns NaN
	  }else if(isZero()&&!arg.isZero()){//return arg if this.isZero
		  return arg;		  
	  }else if(!isZero()&&arg.isZero()){
  ...
  public RatTerm sub(RatTerm arg) {
	  if((expt!=arg.expt)&&!(isNaN()||isZero())&&!(arg.isNaN()||arg.isZero())){// throw exception if two
		  throw new IllegalArgumentException(); // degrees are different, and neither RatTerm is zero or NaN
	  }else if(isNaN()||arg.isNaN()){//If either argument is NaN,returns NaN
		  return new RatTerm(RatNum.NaN, 0);
	  }else if(isZero()&&!arg.isZero()){//if this.isZero,return arg.negate()
		  return arg.negate();		  
	  }else if(!isZero()&&arg.isZero()){
		  return new RatTerm(coeff,expt);
	  }else{
		  return new RatTerm(coeff.sub(arg.coeff),expt);//subtract arg coefficient from this coefficient 
	  }
  ...
  public RatTerm mul(RatTerm arg) {
	  if(isNaN()||arg.isNaN()){//If either argument is NaN, return NaN
		  return new RatTerm(RatNum.NaN, 0);
	  }else{                    // get multiplication by multiplying coefficient and adding degree
   ...
   */
  public RatTerm div(RatTerm arg) {//If arg is zero, or if either argument is NaN, returns NaN
    if(arg.isZero()||isNaN()||arg.isNaN()){
    	return new RatTerm(RatNum.NaN, 0);
    }else{         //get division by dividing the coefficient and subtracting the degree
  ...
  public RatTerm differentiate() {
    if(isNaN()){//if isNaN(),return NaN
    	return new RatTerm(RatNum.NaN, 0);
    }else if(expt==0){// if degree is 0, then return 0
    	return new RatTerm(RatNum.ZERO, 0);
    }else{    //else return the derivative based on mathematical rules
    	return new RatTerm(coeff.mul(new RatNum(expt,1)),expt-1);
    }
  }
  ...
  public RatTerm antiDifferentiate() {
	    if(isNaN()){//if isNaN(), return NaN
	    	return new RatTerm(RatNum.NaN, 0);
	    }else{//else, return anti-derivative based on mathematical rules
	    	return new RatTerm(coeff.div(new RatNum(expt+1,1)),expt+1);
	    }
  }
